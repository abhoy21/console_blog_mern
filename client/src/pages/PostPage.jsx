...
export default function PostPage() {
    ...
    const fetchPost = async () => {
      try {
        ...
        const res = await fetch(
          `https://console-blog-mern-api.vercel.app/api/post/getposts?slug=${postSlug}`,
        );
  ...
  useEffect(() => {
    try {
      const fetchRecentPosts = async () => {
        const res = await fetch(
          `https://console-blog-mern-api.vercel.app/api/post/getposts?limit=3`,
        );
        const data = await res.json();
        if (res.ok) {
  ...
  return (
    <main className='p-3 flex flex-col max-w-6xl mx-auto min-h-screen'>
      ...
      >
        <button className='bg-gradient-to-r from-cyan-300 to-sky-500  rounded-full px-8 py-2 text-lg md:text-xl font-semibold text-white mr-2'>
          {post && post.category}
        </button>
      </a>
      <img
